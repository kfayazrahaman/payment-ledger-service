/**
 * Payment Ledger Service - Main Server File (Mongoose Version)
 * 
 * Key Changes from SQLite Version:
 * - Replaced SQLite initialization with MongoDB/Mongoose connection
 * - Uses connectDatabase() from config/database.js
 * - Removed synchronous database setup
...
async function startServer() {
  try {
    ...
    app.use(
      cors({
        origin: ['http://localhost:3000', 'http://localhost:5173','https://studio.apollographql.com',],
        credentials: true,
      })
    );
    ...
    const server = app.listen(PORT, () => {
      ...
      console.log(`❤️  Health Check: ${BACKEND_URL}/health`);
      console.log('🚀 ========================================\n');
    });
